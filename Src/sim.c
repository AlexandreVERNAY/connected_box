 ...
 */
	launchTimer(TIM4);			// Limit the response time of the SIM module
	USART_TX(UART4, ATcommand);	// Send command to SIM module
	waitForTimeOut(SIM);		// Wait for response to be entirely received
}
 ...
 */
	...
	setTimeout(TIM4, 4000);		// Allow longer delay for SIM module to send the SMS
	SIM_sendCommand(EoTSMS);	// Transmit end of text character to SIM module
	setTimeout(TIM4, 1000);		// Reset time out delay
}
 ...
 */
	...
	extractCommand(LINE_4);		// Extract command from SMS
	SIM_sendCommand(CMGD);		// Erase all messages stored in SIM
}
