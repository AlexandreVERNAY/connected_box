...
struct USART_Handler usart1		= { .buffer[0] = '\0', .size = 0, .timeOut = 0, .command[0] = '\0', .response[0] = '\0' };
struct USART_Handler *SCREEN	= &usart1;

struct USART_Handler uart4		= { .buffer[0] = '\0', .size = 0, .timeOut = 0, .command[0] = '\0', .response[0] = '\0' };
struct USART_Handler *SIM		= &uart4;

uint8_t USART_message[BUFFER_SIZE];				// Buffer between USART modules
uint8_t *USART_messageBuffer = USART_message;
...
//	Generate serial number
	SIM_setIdentity(IDENTITY);					// Generate a unique serial number

//	Create and send an SMS notifying the end of the initialization phase
	createMessage((uint8_t *)SIM_Init_OK, IDENTITY, (uint8_t *)"\0");
	SIM_sendSMS(USART_messageBuffer);			// Send SMS identifying the box

//	Creates Go To Page accueil command for SCREEN
	createMessage((uint8_t *)GTP_accueil, (uint8_t *)GTP_END_COMMAND, (uint8_t*)"\0");
	SCREEN_sendCommand(USART_messageBuffer);	//	Display accueil page in SCREEN
...
//		Before re-entering sleep mode, it executes modules commands
		...
		if(SCREEN->size){						// If SCREEN module has send a command
			SCREEN_executeCommand();			// Execute SCREEN command
		}
