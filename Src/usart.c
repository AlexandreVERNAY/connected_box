...
void createMessage(struct USART_Handler *usart, uint8_t *headMessage, uint8_t *message, uint8_t *tailMessage){
	strcpy((char*)usart->buffer, "");					// Empty the buffer
	strcat((char*)usart->buffer, (char*)headMessage);	// Copy first part into buffer
	strcat((char*)usart->buffer, (char*)message);		// Copy message into buffer
	strcat((char*)usart->buffer, (char*)tailMessage);	// Copy last part into buffer
}
